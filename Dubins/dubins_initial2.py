...
class Dubins(object):
	...
	#tmp = .125 * (6. - d * d + 2. * (ca * cb + sa * sb + d * (sa - sb)))
        if (np.abs(tmp) < 1.):
            print(np.arccos(tmp), 2 * np.pi - np.arccos(tmp), twopify(np.arccos(tmp)))
            p = twopify(np.arccos(tmp))
            theta = np.arctan2(-ca + cb, d + sa - sb)
            t = twopify(-alpha + theta + .5 * p)
            q = twopify(twopify(beta) - alpha - t + p) #twopify(beta - alpha - t + p)
