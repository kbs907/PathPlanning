...
def Parent(G, node_near_id) :
    for id in G.predecessors(node_near_id) :
        return id
...
if __name__ == '__main__':
    ...
    for i in range(max_iterations):
        ...
        if ObstacleFree(node_nearest, node_new, obstacles):
            ...
            for node_near_id in nodes_near_id :
                node_near = G.node[node_near_id]
                cost_new = node_new["cost"] + LineCost(node_new, node_near)
                if ObstacleFree(node_new, node_near, obstacles) and cost_new < node_near["cost"] :
                    node_parent_id = Parent(G, node_near_id)
                    G.remove_edge(node_parent_id, node_near_id)
                    G.add_edge(i, node_near_id)
                    node_near["cost"] = cost_new            
